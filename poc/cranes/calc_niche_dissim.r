...
# chmod 744 calc_niche_dissim.r #Use to make executable
...
Usage:
calc_niche_dissim.r <db> <out> <ctf> <anno> 
calc_niche_dissim.r (-h | --help)
...
if(interactive()) {
  ...
  .outPF <- file.path(.wd,'analysis/cranes/niche_dissims.Rdata')
...
message("Calculating niche dissimilarities...")
ind_ts_dat <- lapply(as.list(unique(anno_mod$individual_id)), FUN = tryCatch(indNDissim), 
